...
$languages = array(
	...
	function fileDescription($filename){
		...
		}elseif($ext == 'zip' OR $ext == 'rar' OR $ext == 'cab' OR $ext == 'b2z' or $ext=="bz2" or $ext=="gz"){
    ...
    // recursively delete or empty a directory or list of directories
    function deleteRecursive($dir, $blnDel = true) {
		if (is_string($dir)) {
			$tmpdir = $dir;
			unset($dir);
			$dir[0] = $tmpdir;
		...
		if (!preg_match("/\.\.?$/", $file)) {
				if (is_dir($file)) {
					deleteRecursive(glob("$file/{,.}*", GLOB_BRACE));
...
/*
	// get $_POST or $_GET variable.
	function getResponse($var) {
		if (isset($_POST[$var])) {
			return $_POST[$var];
